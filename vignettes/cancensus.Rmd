...
Statistics Canada conducts the Census of the Canadian population every five years, with the most recent Census completed in 2016. Statistics Canada [provides](http://www12.statcan.gc.ca/datasets/index-eng.cfm?Temporal=2016) datasets for Census products which are available for the 2016 Census, the 2011 National Household Survey, as well as the 2006, 2001, 1996, and 1991 Censuses at different levels of geography. Statistics Canada provides this data in HTML, XML, and Beyond 20/20 formats.

At this time, Statistics Canada does not provide an API service for Census data nor any other way to programmatically access Census data for reproducible research. 

# Cancensus and CensusMapper

The **cancensus** package was developed to provide users with a way to access Canadian Census in a programmatic way following good [tidy data](http://vita.had.co.nz/papers/tidy-data.pdf) practices. While the structure and data in **cancensus** is unique to Canadian Census data, this package is inspired in part by Kyle Walker's work on [tidycensus](https://github.com/walkerke/tidycensus), a package to interface with the US Census Bureau data APIs. 

As Statistics Canada does not provide API access to Census data, **cancensus** retrieves Census data indirectly through the [CensusMapper](https://censusmapper.ca) API. CensusMapper is a project by [Jens von Bergmann](https://twitter.com/vb_jens), one of the authors of **cancensus**, to provide interactive geographic visualizations of Canadian Census data. CensusMapper databases store all publically available data from Statistics Canada for the 2006, 2011, and 2016 Censuses. Censusmapper data can be accessed via an API and **cancensus** is built to interface directly with it. 
...
```{r list regions, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r list_vectors, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r search_vectors1, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r search_vectors2, message=FALSE, warning=FALSE, eval = FALSE}
# Find the variable indicating the number of people of Austrian ethnic origin
...
```{r parent_vectors, message=FALSE, warning=FALSE, eval=FALSE}
list_census_vectors("CA16") %>% 
...
```{r child_vectors1, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r child_vectors2, message=FALSE, warning=FALSE, eval=FALSE}
# Show all child variable leaves
list_census_vectors("CA16") %>% 
  filter(vector == "v_CA16_4092") %>% child_census_vectors(leaves = TRUE)
```
