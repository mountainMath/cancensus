...
Data returned through the API is cached in order to cut down on API usage, reduce network traffic and increase speed. By default **cancensus** caches data in the package path, and cached data will be purged every time the package is updated. We recommend setting the `cancensus.cache_path` to a custom directory to allow cached data to persist through package updates.

Setting the API key and the cache path in the `.Rprofile` facilitates the ability to share code without exposing API keys or relying on the local directory structure. The `cancensus.cache_path` option simplifies setting up custom cache directories.  
