...
# Interactive maps with mapdeck

The [mapdeck](https://github.com/SymbolixAU/mapdeck) package enables plotting of large spatial datasets on interactive maps using [Mapbox GL](https://www.mapbox.com/about/) and [Deck.gl](http://deck.gl/#/). Mapdeck is fast and has numerous features that make it well suited for working with spatial data. Using mapdeck requires a Mapbox token, with details for setup available [here](https://github.com/SymbolixAU/mapdeck#quick-start). Mapdeck provides extensive additional functionality for displaying spatial data such as visual pitch, 3d-extrusions. 
...
toronto3d <- toronto %>% 
  mutate(adj_income = median_hh_income/10)

mapdeck(token = mb_token, style = mapdeck_style("dark"), pitch = 45 ) %>%
  add_polygon(
    data = toronto3d
    , layer = "polygon_layer"
    , fill_colour = "median_hh_income"
    , elevation = "adj_income"
    , legend = TRUE
  )
```

Mapdeck is especially useful if attempting to visualize large spatial datasets containing many objects, such as Census tracts and dissemination areas, as in this example below looking at the same household income data as above, but using Census tracts instead.  
