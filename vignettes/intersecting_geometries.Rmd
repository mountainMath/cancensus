...
We may increase the default quotas for the `get_intersecting_geometries` call at some point, but while we throttling API usae and monitor server impacts of the new functionality it may be preferable to use the `get_intersecting_geometries` call for higher level geographies only and add a simple line of clode to do the final bit of filtering in R. 
