...
```{r}
cov_stations <- COV_SKYTRAIN_STATIONS %>%
  st_transform(26910) %>%
  st_buffer(800) %>%
  st_transform(4326)
```
...
```{r}
station_city_ids <- get_intersecting_geometries("CA16", level = "CSD", geometry = cov_stations)
station_ct_ids <- get_intersecting_geometries("CA16", level = "CT", geometry = cov_stations)
```
...
ggplot(station_city) +
  ...
  geom_sf(data=cov_stations,fill="brown",alpha=0.5) +
  coord_sf(datum=NA)
```
