...
census_mapper <- function (dataset, level, regions, vectors,api_key, geo=FALSE,force_download=FALSE) {
 ...
 # load data variables
  ...
  dat <- read.csv(data_file, na.strings = 'x', colClasses=c("GeoUID"="character"))
  ...
  if (geo) {
    ...
    if (force_download || !file.exists(geo_file)) {
      ...
      GET(paste(geo_base_url,final_data_param_string,sep='?'),write_disk(geo_file));
...
geos <- census_mapper(dataset='CA16',level='CT',regions=regions_string,vectors=vectors,api_key=api_key,geo=TRUE, force_download = FALSE)
